...
static rtx propeller_function_arg (cumulative_args_t cum, enum machine_mode mode,
                                   const_tree type, bool named);
static void propeller_function_arg_advance (cumulative_args_t cum, enum machine_mode mode,
                                const_tree type, bool named ATTRIBUTE_UNUSED);

static bool propeller_rtx_costs (rtx, int, int, int, int *, bool);
static int propeller_address_cost (rtx, enum machine_mode, addr_space_t, bool);
...
/*
 * flag for turning on/off fcache processing
 */
...
{  /* name, min_len, max_len, decl_req, type_req, fn_type_req, handler, affects_type_identity  */
  { "naked",   0, 0, true, false,  false,  propeller_handle_func_attribute, false },
  { "native",  0, 0, true, false,  false,  propeller_handle_func_attribute, false },
  { "fcache",  0, 0, true, false,  false,  propeller_handle_func_attribute, false },
  { "cogmem",  0, 0, false, false, false, propeller_handle_cogmem_attribute, false },
  ...
  if (cog_p)
    {
      ...
      if (!propeller_cog_globals)
        propeller_cog_globals = save_target_globals_default_opts ();
      else
 ...
 * '~': prints a D if this is a delayed branch sequence (not yet used)
 */
...
propeller_legitimate_address_p (enum machine_mode mode, rtx addr, bool strict)
{
  ...
  if (REG_P (addr)) {
    return INT_REG_OK_FOR_BASE_P (addr, strict);
  }
...
static bool
propeller_rtx_costs (rtx x, int code, int outer_code ATTRIBUTE_UNUSED, int opno ATTRIBUTE_UNUSED, int *total, bool speed)
{
    switch (code) {
    case REG:
    case SUBREG:
    ...
    case SYMBOL_REF:
    case LABEL_REF:
      if (!TARGET_LMM && !TARGET_XMM) {
	*total = 0;
      } else if (propeller_cogaddr_p (x)) {
	*total = 0;
      } else {
	*total = 1;
      }
	...
	*total = COSTS_N_INSNS(1);
      ...
      break;
    case MULT:
      *total = (speed ? COSTS_N_INSNS(16) : COSTS_N_INSNS(3));
      break;
    ...
    case MOD:
    case UMOD:
      *total = (speed ? COSTS_N_INSNS(64) : COSTS_N_INSNS(3));
      break;
	...
	*total += COSTS_N_INSNS(4);  /* and moderately expensive in non-COG mdoes */
      break;
    default:
      *total = COSTS_N_INSNS(1);
      break;
			...
			addr_space_t as ATTRIBUTE_UNUSED, bool speed ATTRIBUTE_UNUSED)
{
  ...
  case CONST:
    ...
    break;
  default:
    /* this is an illegal address, but there are parts of the compiler that
       may query about such addresss, so do not crash */
    total = 99;
    break;
  }
...
static rtx
propeller_function_arg (cumulative_args_t cum_v, enum machine_mode mode,
                   const_tree type, bool named)
{
...
static void
propeller_function_arg_advance (cumulative_args_t cum_v, enum machine_mode mode,
                           const_tree type, bool named ATTRIBUTE_UNUSED)
{
  CUMULATIVE_ARGS *cum = get_cumulative_args (cum_v);
...

bool
propeller_legitimate_constant_p (enum machine_mode mode ATTRIBUTE_UNUSED, rtx x)
{
  switch (GET_CODE (x))
    {
    case LABEL_REF:
...
*/
static GTY(()) tree propeller_builtin[(int) PROPELLER_BUILTIN_max];
...
propeller_init_builtins (void)
{
  ...
  /* void func (void) */
  //tree void_ftype_void = build_function_type (void_type_node, endlink);

  /* void func (unsigned) */
  tree void_ftype_uns = build_function_type (void_type_node, uns_endlink);
  tree void_ftype_uns_uns = build_function_type (void_type_node, uns_uns_endlink);

  /* unsigned func(void) */
  //tree uns_ftype_void = build_function_type (unsigned_type_node, endlink);

  /* unsigned func(unsigned) */
  /* unsigned func(unsigned,unsigned) */
  /* int func(unsigned) */
  //tree uns_ftype_uns = build_function_type (unsigned_type_node, uns_endlink);
  tree uns_ftype_uns_uns = build_function_type (unsigned_type_node, uns_uns_endlink);
  tree int_ftype_uns = build_function_type (integer_type_node, uns_endlink);

  /* int func (void) */
  tree int_ftype_void = build_function_type (integer_type_node, endlink);
  /* void func (int) */
  tree void_ftype_int = build_function_type (void_type_node, int_endlink);
  /* int func (int) */
  tree int_ftype_int = build_function_type (integer_type_node, int_endlink);

  /* void (*)(void) func(void (*f)(void)) */
  //tree vfunc_ftype_vfunc = build_function_type(ptr_type_node, ptr_endlink);

  propeller_builtin[PROPELLER_BUILTIN_CLKSET] =
  add_builtin_function("__builtin_propeller_clkset", void_ftype_uns,
                       PROPELLER_BUILTIN_CLKSET,
                       BUILT_IN_MD, NULL, NULL_TREE);

  propeller_builtin[PROPELLER_BUILTIN_COGID] =
  add_builtin_function("__builtin_propeller_cogid", int_ftype_void,
                       PROPELLER_BUILTIN_COGID,
                       BUILT_IN_MD, NULL, NULL_TREE);

  propeller_builtin[PROPELLER_BUILTIN_COGINIT] =
  add_builtin_function("__builtin_propeller_coginit", int_ftype_uns,
                       PROPELLER_BUILTIN_COGINIT,
                       BUILT_IN_MD, NULL, NULL_TREE);

  propeller_builtin[PROPELLER_BUILTIN_COGSTOP] =
  add_builtin_function("__builtin_propeller_cogstop", void_ftype_int,
                       PROPELLER_BUILTIN_COGSTOP,
                       BUILT_IN_MD, NULL, NULL_TREE);

  propeller_builtin[PROPELLER_BUILTIN_REVERSE] =
  add_builtin_function("__builtin_propeller_rev", uns_ftype_uns_uns,
                       PROPELLER_BUILTIN_REVERSE,
                       BUILT_IN_MD, NULL, NULL_TREE);

  propeller_builtin[PROPELLER_BUILTIN_WAITCNT] =
  add_builtin_function("__builtin_propeller_waitcnt", uns_ftype_uns_uns,
                       PROPELLER_BUILTIN_WAITCNT,
                       BUILT_IN_MD, NULL, NULL_TREE);

  propeller_builtin[PROPELLER_BUILTIN_WAITPEQ] =
  add_builtin_function("__builtin_propeller_waitpeq", void_ftype_uns_uns,
                       PROPELLER_BUILTIN_WAITPEQ,
                       BUILT_IN_MD, NULL, NULL_TREE);

  propeller_builtin[PROPELLER_BUILTIN_WAITPNE] =
  add_builtin_function("__builtin_propeller_waitpne", void_ftype_uns_uns,
                       PROPELLER_BUILTIN_WAITPNE,
                       BUILT_IN_MD, NULL, NULL_TREE);

  propeller_builtin[PROPELLER_BUILTIN_WAITVID] =
  add_builtin_function("__builtin_propeller_waitvid", void_ftype_uns_uns,
                       PROPELLER_BUILTIN_WAITVID,
                       BUILT_IN_MD, NULL, NULL_TREE);

  propeller_builtin[PROPELLER_BUILTIN_LOCKNEW] =
  add_builtin_function("__builtin_propeller_locknew", int_ftype_void,
                       PROPELLER_BUILTIN_LOCKNEW,
                       BUILT_IN_MD, NULL, NULL_TREE);

  propeller_builtin[PROPELLER_BUILTIN_LOCKRET] =
  add_builtin_function("__builtin_propeller_lockret", void_ftype_int,
                       PROPELLER_BUILTIN_LOCKRET,
                       BUILT_IN_MD, NULL, NULL_TREE);

  propeller_builtin[PROPELLER_BUILTIN_LOCKSET] =
  add_builtin_function("__builtin_propeller_lockset", int_ftype_int,
                       PROPELLER_BUILTIN_LOCKSET,
                       BUILT_IN_MD, NULL, NULL_TREE);

  propeller_builtin[PROPELLER_BUILTIN_LOCKCLR] =
  add_builtin_function("__builtin_propeller_lockclr", void_ftype_int,
                       PROPELLER_BUILTIN_LOCKCLR,
                       BUILT_IN_MD, NULL, NULL_TREE);
}

/* return the propeller builtin for code */
static tree
propeller_builtin_decl (unsigned code, bool initialize_p ATTRIBUTE_UNUSED)
{
  if (code >= PROPELLER_BUILTIN_max)
    return error_mark_node;

  return propeller_builtin[code];
}
...
bool
propeller_forward_branch_p (rtx_insn* insn)
{
 ...
 */
static bool
fcache_jump_dest_in_block (rtx jump, rtx_insn* first, rtx_insn* last)
{
  rtx label;
  rtx_insn* insn;
 ...
 */
static int
fcache_label_refs_in_block (rtx lab, rtx_insn* first, rtx_insn* last)
{
  rtx_insn* insn;
  int count = 0;
...
static bool
fcache_block_ok (rtx_insn* first, rtx_insn* last, bool func_p, bool force_p)
{
  rtx dest;
  rtx_insn* insn;
  rtx_insn* last_next;
  ...
  for (insn = first; insn && insn != last_next; insn = NEXT_INSN (insn))
    {
      ...
      else if (INSN_P (insn) && PATTERN(insn) && GET_CODE (PATTERN (insn)) == ASM_INPUT && !force_p)
	{
	  if (print_msgs)
	    {
 ...
 */
static void
fcache_convert_call (rtx_insn* insn)
{
  rtx pattern;
  rtx addr;
  rtx dest;
 ...
 */
static void
fcache_convert_jump(rtx_insn* orig_insn, rtx fcache_base)
{
  rtx pattern;
  rtx src;
  rtx addr;
  ...
  old_label = JUMP_LABEL (orig_insn);
  
  new_insn = emit_jump_insn_before (pattern, orig_insn);

  JUMP_LABEL (new_insn) = old_label;
...
static rtx_insn*
fcache_convert_block (rtx_insn* first, rtx_insn* last, bool func_p)
{
  ...
  start_label_ref = gen_rtx_LABEL_REF (VOIDmode, start_label);
  ...
  end_label_ref = gen_rtx_LABEL_REF (VOIDmode, end_label);

  /* now add in the FCACHE_LOAD */
  fcache_rtx = gen_rtx_UNSPEC_VOLATILE (VOIDmode, gen_rtvec (2, start_label_ref, end_label_ref),
                         UNSPEC_FCACHE_LOAD);

  fcache_insn = emit_insn_before (fcache_rtx, first);
  ...
  */
  if (func_p)
    {
      first = emit_insn_after (gen_fcache_func_start (), first);
      INSN_ADDRESSES_NEW (first, -1);
      last = emit_insn_after (gen_fcache_done_func (), last);
      INSN_ADDRESSES_NEW (last, -1);
    }
  /* if not a function, insert a return back to the fcache handler at the end */
  else
    {
      last = emit_insn_after (gen_fcache_done(), last);
      INSN_ADDRESSES_NEW (last, -1);
    }
  ...
  for (insn = first; insn && insn != last_next; insn = NEXT_INSN (insn))
    {
      ...
      if (CALL_P (insn))
        ...
        {
          fcache_convert_jump (insn, start_label_ref);
        }
      else
        {
          pattern = PATTERN (insn);
          if ( GET_CODE (pattern) == SET
            ...
            
            {
              ...
              */
              ...
              /* update the pattern */
              SET_SRC (pattern) = gen_rtx_UNSPEC (mode, gen_rtvec (2, const_label, start_label_ref), UNSPEC_FCACHE_LABEL_REF);
              emit_insn_before (pattern, mov_insn);
              /* delete the old one */
              delete_insn (mov_insn);
            }
        }
    }
  return fcache_insn;
 ...
 */
static bool
try_convert_loop (rtx_insn **first_ptr, rtx_insn **last_ptr)
{
  rtx_insn* first = *first_ptr;
  rtx_insn* last = *last_ptr;
  rtx_insn* prev;
  rtx_insn* next;
...
int
propeller_reg_dead_peep (const rtx_insn* first, rtx reg)
{
  rtx_insn *insn;

  /* For us, subregs can't live independently of their parent regs.  */
...

/* return TRUE if we should use LRA instead of reload */
static bool
propeller_lra_p (void)
{
  return TARGET_EXPERIMENTAL;
}

...
#define TARGET_LEGITIMATE_CONSTANT_P propeller_legitimate_constant_p
...
#define TARGET_BUILTIN_DECL propeller_builtin_decl
...
#undef TARGET_LRA_P
#define TARGET_LRA_P propeller_lra_p
